...
## 3.7 Parked workstream — per-sample AGC noise modulation (Brent + Codex, 2026-05-04)

Brent independently investigated the click symptom from the AGC
side and converged with Codex on a distinct mechanism that runs
in parallel with the audio rebuild's cadence-mismatch story.
Captured here so the finding doesn't get lost; explicitly NOT
addressed during the audio rebuild (Commits 1-6) so we don't
context-switch mid-architecture-fix.

### The mechanism

`Radio._apply_agc_and_volume` runs a per-sample envelope
tracker on `np.abs(audio)` -- half-rectified post-demod real
audio, NOT the complex envelope that WDSP's wcpAGC operates
on.  Each sample on Gaussian noise can cross the tracked peak
and trigger an instant "attack" (`peak[n] = m`), which resets
gain to `target / peak`.

At the Med preset (release=0.158, hang=0), the equilibrium peak
sits at ~3.1σ of the noise distribution, producing roughly
**100 attacks/sec** on noise-only audio.  Each attack drops gain
~8% (~0.7 dB), then ~116 ms exponential recovery.  The
overlapping recoveries sum to a Lorentzian-spectrum AM
modulation of the noise floor: broadband random texture, not
tonal -- which matches the operator's "scratchy / dirty record
player" description perfectly.

### Why Thetis is clean here

WDSP's wcpAGC operates on the **complex envelope**
(`sqrt(I² + Q²)`), which is Rayleigh-smooth rather than per-
sample-noisy half-Gaussian.  Plus it has a look-ahead delay
ring buffer so attacks ramp instead of stepping.  Two reasons
the same textural artifact doesn't appear in Thetis on the
same hardware.

### Falsifiable test (operator can run anytime)

Switch AGC profile to **Slow**.  At 490 ms hang, equilibrium
peak rises to ~4.0σ and attack rate drops from ~100/sec to
~5-10/sec.

- If Slow sounds noticeably less scratchy than Med (probably
  with mild slow pumping or occasional ticks instead of
  continuous hash) → mechanism confirmed.
- If Slow sounds identical to Med → hypothesis falsified.

30-second test.  No code change.  Independent of the audio
rebuild's progress.

### Agreed fix (when we get to it)

Dynamic `PEAK_FLOOR` clamped to `K × self._noise_baseline` for
K ≈ 1.5-2.0.  Lyra already tracks `_noise_baseline` from the
rolling min of `peak_arr` (radio.py:5879-5884).  Effect:
during noise-only listening, peak can't decay below the noise
envelope's typical max → no "attack-able room" → no AM
modulation.  Real signals (>>1.5× noise) still trigger attacks
at full speed → CW dit edges and SSB consonant onsets
preserved.

Codex initially proposed "finite-slew attack with ~1-2 ms
window" but switched to the dynamic-floor approach in round 2
review -- more targeted (slew universally brakes attacks;
dynamic floor only kills the noise-modulation failure mode).

Estimated implementation: ~5 lines in `_apply_agc_and_volume`,
plus a constant for K.  Trivial.

### Sequencing rationale

Two independent click-source mechanisms, two independent fixes:

1. **EP2 underrun zero-pad injection** (audio rebuild Commits
   1-6).  Discrete click events at producer/consumer cadence
   mismatch.
2. **Per-sample AGC attack on noise** (this section).
   Continuous textural artifact in the noise floor.

Audio rebuild ships first because:
- The rebuild gives us a clean architectural baseline (no
  cadence-mismatch underruns) against which to evaluate AGC
  noise-modulation isolation.
- Mid-rebuild context-switching risks regressing both fixes.
- Operator can run the Slow-preset test anytime to validate
  Brent's hypothesis on the existing build, independent of
  rebuild progress.

After the audio rebuild lands fully and both testers sign off,
the AGC fix becomes a small targeted commit (probably v0.0.9.3
or rolled into the v0.1 RX2 release).  Brent owns this one.
